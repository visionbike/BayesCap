...
class ResConv(nn.Module):
    """
    Residual convolutional block, where convolutional block consists: (convolution => [BN] => ReLU) * 3
...
class Down(nn.Module):
    """
    Downscaling with max-pooling then residual conv
    """
...
class UNet(nn.Module):
    def __init__(self, n_channels: int, out_channels: int, bilinear: bool = True) -> None:
        """
...
class CasUNet(nn.Module):
    def __init__(self, n_unet: int, io_channels: int, bilinear: bool = True) -> None:
        """
    ...
    def forward(self, x: torch.Tensor, dop: float | None = None) -> torch.Tensor:
        ...
        for i in range(self.n_unet):
            if i == 0:
                if dop is not None:
                    y = nfn.dropout2d(self.unet_list[i](y), p=dop)
                else:
...
class MTResnetGenerator(nn.Module):
    """
    ...
    def __init__(self,
                 ...
                 output_nc: int,
                 ngf: int = 64,
                 norm_layer: nn.Module | functools.partial = nn.BatchNorm2d,
                 use_dropout: bool = False,
                 n_blocks: int = 6,
                 padding_type: str = "reflect") -> None:
        """
        ...
        :param use_dropout: whether dropout layers are used.
        :param n_blocks: the number of ResNet blocks.
        :param padding_type: the name of padding layer in conv layers: reflect | replicate | zero.
        """
        assert (n_blocks >= 0)
        super().__init__()
...
class ResnetBlock(nn.Module):
    ...
    def __init__(self, dim: int, padding_type: str, norm_layer: nn.Module | functools.partial, use_dropout: bool, use_bias: bool) -> None:
        """
    ...
    def build_conv_block(self, dim: int, padding_type: str, norm_layer: nn.Module | functools.partial, use_dropout: bool, use_bias: bool) -> nn.Module:
        """
        ...
        :param norm_layer: normalization layer.
        :param use_dropout: if dropout layers are used.
        :param use_bias: if the conv layer uses bias or not.
        :return: a conv block (with a conv layer, a normalization layer, and a non-linearity layer (ReLU)).
        """
        conv_block = []
        p = 0
...
# discriminator
class MTNLayerDiscriminator(nn.Module):
    """
    Defines a PatchGAN discriminator
    """

    def __init__(self, input_nc: int, ndf: int = 64, n_layers: int = 3, norm_layer: nn.Module | functools.partial = nn.BatchNorm2d) -> None:
        """
